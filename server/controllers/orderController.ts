    ...
    if (!user.is_verified) {
      res
        ...
        .json({
          success: false,
          message:
            "Your account must be verified before placing orders. Please check your email for the verification link.",
        });
      return;
    }
...
export const getOrderById = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (order.userId !== req.user!.id) {
            res.status(403).json({ success: false, message: "You can only access your own orders" });
            return;
        }
